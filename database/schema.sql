...
CREATE INDEX IF NOT EXISTS idx_properties_failure_count ON properties(failure_count);
CREATE INDEX IF NOT EXISTS idx_properties_building_area ON properties(building_area);

-- 복합 인덱스 (검색 성능 향상)
CREATE INDEX IF NOT EXISTS idx_properties_status_date ON properties(current_status, auction_date);
CREATE INDEX IF NOT EXISTS idx_properties_type_status ON properties(property_type, current_status);
CREATE INDEX IF NOT EXISTS idx_properties_price_range ON properties(minimum_sale_price, appraisal_value);
...
CREATE INDEX IF NOT EXISTS idx_analysis_results_grade ON analysis_results(investment_grade);
CREATE INDEX IF NOT EXISTS idx_analysis_results_roi ON analysis_results(roi_1year);
CREATE INDEX IF NOT EXISTS idx_analysis_results_risk_level ON analysis_results(risk_level);

-- 복합 인덱스 (분석 결과 검색 최적화)
CREATE INDEX IF NOT EXISTS idx_analysis_score_grade ON analysis_results(investment_score, investment_grade);
CREATE INDEX IF NOT EXISTS idx_analysis_roi_risk ON analysis_results(roi_1year, risk_level);

-- Market Trends 인덱스
CREATE INDEX IF NOT EXISTS idx_market_trends_region ON market_trends(region_code);
CREATE INDEX IF NOT EXISTS idx_market_trends_property_type ON market_trends(property_type);
CREATE INDEX IF NOT EXISTS idx_market_trends_period ON market_trends(analysis_period);
CREATE INDEX IF NOT EXISTS idx_market_trends_date ON market_trends(analysis_date);
...
LEFT JOIN regions r ON ps.address LIKE CONCAT('%', r.sigungu, '%')
LEFT JOIN market_trends mt ON r.sigungu = mt.region_code 
